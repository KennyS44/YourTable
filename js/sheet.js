...
export const GUARDS = [['ac', 'Бросок атаки по КД'], ['save', 'Спасбросок цели'], ['none', 'Без проверки']];
...
/* Корни видов урона: в листе пишут живым языком — «стоек к огню», «боится
   холода». Ищем по корню, потому что окончание каждый раз своё. */
const DMG_STEMS = {
  'рубящий': ['рубящ'], 'колющий': ['колющ'], 'дробящий': ['дробящ'],
  'огонь': ['огн', 'огон'], 'холод': ['холод'], 'молния': ['молни'],
  'кислота': ['кислот'], 'яд': ['яд'], 'некротический': ['некрот'],
  'излучение': ['излуч'], 'психический': ['психич'], 'силовое': ['силов'],
  'звук': ['звук'],
};

/** Какие виды урона названы в тексте. Порядок — как в DMG_TYPES. */
 ...
 */
export function savesFromSheet(s) {
  const out = {};
  ABILITIES.forEach((a) => { out[a.id] = mod(s[a.id]); });
  return out;
}
